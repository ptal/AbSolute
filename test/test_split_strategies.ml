...
let test_middle () =
  ...
  begin
    ...
    let octagon = OctagonZ.incremental_closure octagon (List.hd (Rewriter.rewrite r (Cmp (Var "x", LEQ, Cst (Bound_rat.of_int 2, Int))))) in
    let middle = Middle.select (OctagonZ.unwrap octagon) Test_rewriter.x_i in
    Alcotest.(check bool) ("middle of [2..4] is " ^ (Z.to_string middle)) true (Z.equal middle (Z.of_int_up 3));
    let octagon = OctagonZ.incremental_closure octagon (List.hd (Rewriter.rewrite r (Cmp (Var "x", LEQ, Cst (Bound_rat.of_int 1, Int))))) in
    let middle = Middle.select (OctagonZ.unwrap octagon) Test_rewriter.x_i in
