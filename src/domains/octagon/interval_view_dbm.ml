...
struct
  ...
  let sqrt2_it = I.create (OF_RATS ((R.sqrt_down R.two),(R.sqrt_up R.two)))

  let dbm_to_lb = wrap lb_rounding (fun v b ->
    if is_rotated v then
      I.lb (Bot.nobot (I.div (I.create (OF_RAT b)) sqrt2_it))
    else
      R.div b R.two)

  let dbm_to_ub = wrap ub_rounding (fun v b ->
    if is_rotated v then
      I.ub (Bot.nobot (I.div (I.create (OF_RAT b)) sqrt2_it))
    else
      R.div b R.two)
  ...
  let lb_to_dbm = wrap B.of_rat_up (fun v b ->
    if is_rotated v then
      I.ub (I.binop MUL (I.create (OF_RAT b)) sqrt2_it)
    else
      R.mul b R.two)
