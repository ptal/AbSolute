...
module Representation (D:ADomain) = struct
  ...
  let empty = Abstract1.top man empty_env

  let extend elm (var, id) = Abstract1.change_environment man elm
  ...
  let apron_to_bexpr abs =
    ...
    Array.map (fun t ->
        apron_to_bexpr {tcons0 = t; Tcons1.env = tenv} tenv
      ) earray
    |> Array.to_list
  ...
  let relax : t -> Csp.bconstraint -> rconstraint list = assert false
end
...
module MAKE(AP:ADomain) = struct
  ...
  let filter b (e1,c,e2) =
    let env = A.env b in
    let c = R.cmp_expr_to_tcons (e1,c,e2) env in
  ...
  let forward_eval abs cons = assert false
    (* let ap_expr = R.expr_to_apron abs cons |> Texpr1.of_expr (A.env abs) in
