...
module Make(B: Bound_sig.S) = struct
  ...
  let meet ((l1,h1):t) ((l2,h2):t) : t bot =
    to_bot (max_low l1 l2, min_up h1 h2)
  ...
  let merge_check a b c d =
    merge_bot2 (to_bot (a,b)) (to_bot (c,d))
  ...
  let filter_tan i r =
    ...
    let aux = div (add i i_pi_half) i_pi in
    (*Format.printf "atan = %s\n aux = %s\n" (to_string atan_r) (bot_to_string to_string aux);*)
    match aux with
    | Bot -> Bot
    | Nb (p1,p2) ->
      let idx = ref ((int_of_float (B.to_float_up (B.floor (snd p1)))) - 1) in
  ...
  (* r = cot i => i = arccot r *)
  let filter_cot _ _ = failwith "todo filter_cot"
  ...
  let filter_min (l1, u1) (l2, u2) (lr, ur) =
    merge_bot2 (to_bot (bf B.max l1 lr, bf B.max u1 ur)) (to_bot (bf B.max l2 lr, bf B.max u2 ur))

  (* r = max (i1, i2) *)
  let filter_max (l1, u1) (l2, u2) (lr, ur) =
    merge_bot2 (to_bot (bf B.min l1 lr, bf B.min u1 ur)) (to_bot (bf B.min l2 lr, bf B.min u2 ur))
