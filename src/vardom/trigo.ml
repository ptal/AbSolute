...
(*
  This modules defines sound operators for trigonometrical functions
...
module Make (I:Vardom_sig.Vardom_sig) = struct
  ...
  let sin_itv i =
    cos_itv (I.binop SUB i pihalf_itv)

  (* interval asin (arcos + arcsin = pi/2) *)
  let asin_itv i = lift_bot (I.binop SUB pihalf_itv) (acos_itv i)
  ...
  let arc return_range fun_itv =
    let other = I.binop ADD return_range pi_itv in
    fun itv result ->
    (* handling of the symetry *)
    match (I.meet itv return_range),(I.meet itv other) with
    | Bot,Bot -> Bot
    | Nb _,Bot  -> fun_itv result
    | Bot,Nb _  -> lift_bot (I.binop ADD pi_itv) (fun_itv (I.unop NEG result))
    | Nb _,Nb _   ->
       lift_bot (I.binop ADD pi_itv) (fun_itv (I.unop NEG result))
  ...
  let filter domain_range fun_itv =
    let other = I.binop ADD twopi_itv domain_range in
    ...
    try
      ...
      let first_part =
        ...
        | Bot -> Bot
        | Nb i' -> lift_bot (I.binop ADD delta) (fun_itv i' r)
      in
      let second_part =
        match (I.meet i' other) with
        | Nb x ->
           let x' = I.binop SUB x twopi_itv in
           lift_bot (I.binop ADD (I.binop ADD delta twopi_itv)) (fun_itv x' r)
        | Bot -> Bot
      in
  ...
  let filter_tan (i:I.t) (r:I.t) : I.t bot =
    try
      ...
      lift_bot (I.binop ADD delta) (I.meet i' (atan_itv r))
    with
    | Exit -> Nb i
